...
def will_be_within_bounds(name: str, linear_velocity: tuple, angular: float) -> tuple:
    ...
    rospy.loginfo(f"Checking if {name} will be within bounds after publishing a twist command.")
    ...
    if not in_bounds:
        return False, f"This command will move the turtle out of bounds to ({new_x}, {new_y})."
...
def publish_twist_to_cmd_vel(
    ...
    try:
        ...
        for _ in range(steps):
            pub.publish(vel)
            rospy.sleep(1)
        rospy.loginfo(f"Published Twist (/{name}/cmd_vel): linear=({velocity}, {lateral}), angular={angle}.")
    except Exception as e:
        return f"Failed to publish {vel} to /{name}/cmd_vel: {e}"
    finally:
        current_pose = get_turtle_pose.invoke({"names": [name]})
        return (f"New Pose ({name}): x={current_pose[name].x}, y={current_pose[name].y}, "
...
def reset_turtlesim():
    ...
    try:
        ...
        global cmd_vel_pubs
        cmd_vel_pubs.clear()
        cmd_vel_pubs["turtle1"] = rospy.Publisher(f'/turtle1/cmd_vel', Twist, queue_size=10)
