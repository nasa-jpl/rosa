...
from rosa.rosa import ROSAConfigurationError
...
class TestROSAHistoryTrimming(unittest.TestCase):
    ...
    def test_trim_chat_history_even_messages_basic(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            trimmed = rosa._trim_chat_history(chat_history)
...
class TestROSAHistoryManagementMethods(unittest.TestCase):
    ...
    def test_clear_chat_retain_system_messages_true(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            self.assertEqual(len(rosa._ROSA__chat_history), 2)
            self.assertEqual(rosa._ROSA__chat_history[0], system_msg1)
            self.assertEqual(rosa._ROSA__chat_history[1], system_msg2)
    ...
    def test_get_history_length_with_messages(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            for i in range(1, 11):
                rosa._ROSA__chat_history = []
                for j in range(i):
                    rosa._ROSA__chat_history.append(HumanMessage(content=f"Query {j}"))
                    rosa._ROSA__chat_history.append(AIMessage(content=f"Response {j}"))
    ...
    def test_trim_history_public_method(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            rosa._ROSA__chat_history = []
            for i in range(10):
                rosa._ROSA__chat_history.append(HumanMessage(content=f"Query {i}"))
                rosa._ROSA__chat_history.append(AIMessage(content=f"Response {i}"))
            
            # Trim to 6 messages
            rosa.trim_history(6)
            
            # Should have 6 messages (3 pairs)
            self.assertEqual(len(rosa._ROSA__chat_history), 6)
            
            # Should be the most recent messages
            self.assertEqual(rosa._ROSA__chat_history[0].content, "Query 7")
            self.assertEqual(rosa._ROSA__chat_history[1].content, "Response 7")
            self.assertEqual(rosa._ROSA__chat_history[4].content, "Query 9")
            self.assertEqual(rosa._ROSA__chat_history[5].content, "Response 9")
    ...
    def test_trim_history_larger_than_current(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            rosa._ROSA__chat_history = original_history.copy()
            
            # Trim to larger size
            rosa.trim_history(10)
            
            # Should remain unchanged
            self.assertEqual(len(rosa._ROSA__chat_history), 4)
            self.assertEqual(rosa._ROSA__chat_history, original_history)
...
class TestROSAHistoryRecordingIntegration(unittest.TestCase):
    ...
    def test_record_chat_history_triggers_trimming(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            rosa._record_chat_history("Query 2", "Response 2")
            
            # Should have exactly 4 messages (limit reached, but no trimming yet)
            self.assertEqual(len(rosa._ROSA__chat_history), 4)
            
            # Record another conversation (should trigger trimming)
            rosa._record_chat_history("Query 3", "Response 3")
            
            # Should still have 4 messages (oldest pair removed)
            self.assertEqual(len(rosa._ROSA__chat_history), 4)
            
            # Should be the most recent messages
            self.assertEqual(rosa._ROSA__chat_history[0].content, "Query 2")
            self.assertEqual(rosa._ROSA__chat_history[1].content, "Response 2")
            self.assertEqual(rosa._ROSA__chat_history[2].content, "Query 3")
            self.assertEqual(rosa._ROSA__chat_history[3].content, "Response 3")
    ...
    def test_record_chat_history_disabled_accumulation(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            rosa = ROSA(ros_version=1, llm=self.mock_llm, 
                       accumulate_chat_history=False, max_history_length=4)
            ...
            self.assertEqual(len(rosa._ROSA__chat_history), 2)
            self.assertEqual(rosa._ROSA__chat_history[0].content, "Query 1")
            self.assertEqual(rosa._ROSA__chat_history[1].content, "Response 1")
    ...
    def test_record_chat_history_unlimited_length(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            for i in range(10):
                rosa._record_chat_history(f"Query {i}", f"Response {i}")
            
            # Should have all 20 messages
            self.assertEqual(len(rosa._ROSA__chat_history), 20)
            
            # Verify all messages are present
            for i in range(10):
                self.assertEqual(rosa._ROSA__chat_history[i*2].content, f"Query {i}")
                self.assertEqual(rosa._ROSA__chat_history[i*2+1].content, f"Response {i}")
    ...
    def test_record_chat_history_atomic_operations(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            self.assertEqual(len(rosa._ROSA__chat_history) % 2, 0)
            
            # Verify message types alternate correctly
            for i in range(0, len(rosa._ROSA__chat_history), 2):
                self.assertIsInstance(rosa._ROSA__chat_history[i], HumanMessage)
                self.assertIsInstance(rosa._ROSA__chat_history[i+1], AIMessage)
    ...
    def test_record_chat_history_error_handling(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            # Mock the _trim_chat_history method to raise an exception
            original_trim = rosa._trim_chat_history
            
            def failing_trim(history):
                if len(history) > 2:  # Fail when trying to trim
                    raise RuntimeError("Trimming failed")
                return original_trim(history)
            
            with patch.object(rosa, '_trim_chat_history', side_effect=failing_trim):
                # This should not raise an exception, but should log the error
                rosa._record_chat_history("Query 1", "Response 1")  # Should work
                
                # This should handle the trimming error gracefully
                with patch.object(rosa, '_ROSA__logger') as mock_logger:
                    rosa._record_chat_history("Query 2", "Response 2")  # Should trigger error
...
class TestROSAHistoryPerformanceAndStress(unittest.TestCase):
    ...
    def test_performance_with_large_history(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            self.assertLess(add_time, 5.0, f"Adding 1000 messages took {add_time:.2f}s, should be < 5s")
            
            # Verify we have exactly 1000 messages
            self.assertEqual(len(rosa._ROSA__chat_history), 1000)
            ...
            self.assertLess(trim_time, 1.0, f"Trimming 1000->100 messages took {trim_time:.2f}s, should be < 1s")
            
            # Verify trimming worked correctly
            self.assertEqual(len(rosa._ROSA__chat_history), 100)
            ...
            self.assertLess(usage_time, 0.5, f"get_history_usage() took {usage_time:.2f}s, should be < 0.5s")
    ...
    def test_memory_leak_detection(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            for cycle in range(100):  # 100 cycles of operations
                ...
                for i in range(10):
                    rosa._record_chat_history(f"Cycle {cycle} Query {i}", f"Cycle {cycle} Response {i}")
                
                # Clear and reload history
                rosa.clear_chat()
                for i in range(5):
                    rosa._record_chat_history(f"Reload {cycle}.{i}", f"Response {cycle}.{i}")
            ...
            self.assertEqual(len(rosa._ROSA__chat_history), 4)  # Last trim_history(4)
            ...
            rosa_still_exists = any(id(obj) == rosa_ref for obj in current_objects if hasattr(obj, '__class__'))
            # Note: This check might not always work due to test framework references
            # self.assertFalse(rosa_still_exists, "ROSA object may not have been garbage collected")
    ...
    def test_concurrent_access_chat_history(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            expected_results = ['worker_0', 'worker_1', 'reader_0', 'reader_1', 'manager_0']
            ...
            if final_length > 0:
                self.assertEqual(final_length % 2, 0, "History should contain complete message pairs")
    ...
    def test_stress_rapid_operations(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            for i in range(200):
                ...
                rosa._record_chat_history(f"Rapid query {i}", f"Rapid response {i}")
                
                # Every few iterations, perform various operations
                if i % 5 == 0:
                    length = rosa.get_history_length()
                    self.assertLessEqual(length, 20, f"History exceeded limit: {length}")
            ...
            self.assertLess(total_time, 3.0, f"Stress test took {total_time:.2f}s, should be < 3s")
            ...
            if len(history) > 0:
                self.assertEqual(len(history) % 2, 0, "History should contain complete pairs")
    ...
    def test_trim_chat_history_empty_list(self):
        ...
        with patch('rosa.rosa.ROSATools') as mock_tools, \
            ...
            trimmed = rosa._trim_chat_history([])
